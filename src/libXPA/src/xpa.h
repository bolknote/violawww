...
extern xpa_menu xpa_create ARGS((Display * disp, char* title, xpa_entry* entrys, int mask,
                                 xpa_appearance* appearance));

#define XPA_PRESS 0x01

extern void xpa_post ARGS((xpa_menu menu, int x, int y, int depth, int* vals, int opt));

extern void xpa_unpost ARGS((xpa_menu menu));

#define XPA_NOGRAB -3
#define XPA_FOREIGN -2
#define XPA_HANDLED -1

extern int xpa_filter ARGS((XEvent * evt, xpa_menu* menu, int** result));

extern int xpa_moded ARGS((xpa_menu menu, int x, int y, int depth, int* vals, int opt,
                           int (*handler)(), int** result));

extern void xpa_destroy ARGS((xpa_menu menu));

extern char* xpa_error ARGS((int code));

extern void xpa_set_error ARGS((void (*func)()));

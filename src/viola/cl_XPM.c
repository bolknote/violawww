...
SlotInfo cl_XPM_CSlots[] = {{STR_class, PTRS | SLOT_RW, (long)"XPM"},
                            {
                                STR_classScript,
                                PTRS,
                                (long)"\n\
	...
	",
                            ...
                            {0}};
SlotInfo cl_XPM_PSlots[] = {{STR__classInfo, CLSI, (long)&class_XPM}, {0}};

SlotInfo* slots_XPM[] = {(SlotInfo*)cl_XPM_NCSlots, (SlotInfo*)cl_XPM_NPSlots,
                         (SlotInfo*)cl_XPM_CSlots, (SlotInfo*)cl_XPM_PSlots};

MethodInfo meths_XPM[] = {
    /* local methods */
    {STR_config, meth_XPM_config},
    {
        STR_freeSelf,
        meth_XPM_freeSelf,
    },
    ...
    {STR_initialize, meth_XPM_initialize},
    {STR_render, meth_XPM_render},
    {STR_seta, meth_XPM_set},
    {0}};

ClassInfo class_XPM = {
    helper_XPM_get, helper_XPM_set, slots_XPM, /* class slot information	*/
    meths_XPM,                                 /* class methods		*/
    STR_XPM,                                   /* class identifier number	*/
    &class_field,                              /* super class info		*/
};

long meth_XPM_config(VObj* self, Packet* result, int argc, Packet argv[]) {
    return meth_field_config(self, result, argc, argv);
}

long meth_XPM_freeSelf(VObj* self, Packet* result, int argc, Packet argv[]) {
    if (!exitingViola) {
        if (GET__label(self)) {
...
long meth_XPM_get(VObj* self, Packet* result, int argc, Packet argv[]) {
    return helper_XPM_get(self, result, argc, argv, getIdent(PkInfo2Str(argv)));
}

long meth_XPM_initialize(VObj* self, Packet* result, int argc, Packet argv[]) {
...
long helper_XPM_set(VObj* self, Packet* result, int argc, Packet argv[], int labelID) {
    ...
    case STR_label: {
        ...
        pixmap = GLMakeXPMFromASCII(rootWindow, result->info.s, &width, &height, &hotx, &hoty);
        if (GET__label(self))
            XFreePixmap(display, (Pixmap)GET__label(self));
...
long meth_XPM_set(VObj* self, Packet* result, int argc, Packet argv[]) {
    return helper_XPM_set(self, result, argc, argv, getIdent(PkInfo2Str(argv)));
}

long meth_XPM_render(VObj* self, Packet* result, int argc, Packet argv[]) {
    Window w = GET_window(self);
    Pixmap pixmap;

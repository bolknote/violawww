...
HTAnchor* HTHistory_backtrack NOARGS /* FIXME: Should we add a `sticky' option ? */
{
...
BOOL HTHistory_canBacktrack NOARGS { return (HTList_objectAt(history, 1) != NULL); }
...
HTAnchor* HTHistory_moveBy ARGS1(int, offset) {
    ...
    if (last != (HTAnchor*)last->parent) { /* Was a child */
        ...
        if (nextOne) {
            ...
            if (TRACE)
                fprintf(stderr, "HTHistory_moveBy: offset by %+d goes out of list %p.\n", offset,
                        (void*)kids);
            return NULL;
        }
    } else {         /* Was a parent */
...
HTAnchor* HTHistory_recall ARGS1(int, number) {
    HTAnchor* destination = HTList_objectAt(history, HTList_count(history) - number);
...
void HTHistory_leavingFrom ARGS1(HTAnchor*, anchor) {
    if (HTList_removeLastObject(history))
        ...
        fprintf(stderr, "HTHistory_leavingFrom: empty history !\n");
}

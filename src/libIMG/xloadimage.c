...
char* argv[];
{
    ...
    for (a = 1; a < argc; a++) {
        ...
        case OA_AT:
            if (!argv[++a])
                break;
            if (sscanf(argv[a], "%d,%d", &images[imagecount].atx, &images[imagecount].aty) != 2) {
        ...
        case OA_CLIP:
            if (!argv[++a])
                break;
            if (sscanf(argv[a], "%d,%d,%d,%d", &images[imagecount].clipx, &images[imagecount].clipy,
    ...
    for (a = 0; a < imagecount; a++) {
        ...
        switch (imageInWindow(disp, scrn, dispimage, user_geometry, fullscreen, install,
                              private_cmap, fit, use_pixmap, images[a].delay, visual_class, argc,
                              argv, verbose)) {
    ...
    }
    {
        ...
        for (i = 0; i < imgObjIdx; i++) {
            ...
            attrs.event_mask = StructureNotifyMask | KeyPressMask | ButtonPressMask |
                               ButtonReleaseMask | ExposureMask;

            imgObj[i].w = XCreateWindow(disp, RootWindow(disp, scrn), (int)10 + (i * 50), (int)10,
                                        (int)imgObj[i].image->width, (int)imgObj[i].image->height,
...
void processEvent(event) XEvent* event;
{
    int i, x, y;
    ...
    case Expose: {
        ...
        for (i = 0; i < imgObjIdx; i++) {
            if (imgObj[i].w == w) {
                ...
                XPutImage(Disp, w, gc, imgObj[i].ximageinfo->ximage, 0, 0, 0, 0,
                          imgObj[i].image->width, imgObj[i].image->height);
                XFlush(Disp);
            }
        }
    }

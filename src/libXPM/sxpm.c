...
static char* plaid[] = {
    ...
    "22 22 4 2",
    /* colors */
    "   c red 	m white  s light_color", "Y  c green	m black  s lines_in_mix",
    "+  c yellow	m white  s lines_in_dark", "x 		m black  s dark_color",
    /* pixels */
    "x   x   x x x   x   x x x x x x + x x x x x ", "  x   x   x   x   x   x x x x x x x x x x x ",
    "x   x   x x x   x   x x x x x x + x x x x x ", "  x   x   x   x   x   x x x x x x x x x x x ",
    "x   x   x x x   x   x x x x x x + x x x x x ", "Y Y Y Y Y x Y Y Y Y Y + x + x + x + x + x + ",
    "x   x   x x x   x   x x x x x x + x x x x x ", "  x   x   x   x   x   x x x x x x x x x x x ",
    "x   x   x x x   x   x x x x x x + x x x x x ", "  x   x   x   x   x   x x x x x x x x x x x ",
    "x   x   x x x   x   x x x x x x + x x x x x ", "          x           x   x   x Y x   x   x ",
    "          x             x   x   Y   x   x   ", "          x           x   x   x Y x   x   x ",
    "          x             x   x   Y   x   x   ", "          x           x   x   x Y x   x   x ",
    "x x x x x x x x x x x x x x x x x x x x x x ", "          x           x   x   x Y x   x   x ",
    "          x             x   x   Y   x   x   ", "          x           x   x   x Y x   x   x ",
    "          x             x   x   Y   x   x   ", "          x           x   x   x Y x   x   x "};
...
main(argc, argv) unsigned int argc;
char** argv;
{
    int ErrorStatus;
    ...
    topw = XtInitialize(argv[0], "Sxpm", options, XtNumber(options), &argc, argv);
    ...
    if (XrmGetResource(xrdb, NULL, "sxpm.geometry", &stype, &val) ||
        ...
        geo = (char*)val.addr;
        flags = XParseGeometry(geo, &x_rtn, &y_rtn, (unsigned int*)&w_rtn, (unsigned int*)&h_rtn);
    ...
    if (iconFile) {
        ...
        iconW = XCreateSimpleWindow(dpy, root, 0, 0, IWIDTH, IHEIGHT, 1, bpix, bpix);

        icon.attributes.valuemask = XpmReturnPixels;
        ErrorStatus =
            XpmReadFileToPixmap(dpy, root, iconFile, &icon.pixmap, &icon.mask, &icon.attributes);
    ...
    ErrorStatus =
        XpmCreatePixmapFromData(dpy, win, plaid, &view.pixmap, &view.mask, &view.attributes);
    ErrorMessage(ErrorStatus, "Plaid");

    ErrorStatus = XpmCreateDataFromPixmap(dpy, &data, view.pixmap, view.mask, &view.attributes);
    ...
    XFreeColors(dpy, colormap, view.attributes.pixels, view.attributes.npixels, 0);
    ...
    if (input || stdinf) {
        ...
        view.attributes.valuemask |= XpmReturnPixels;
        ErrorStatus =
            XpmReadFileToPixmap(dpy, win, input, &view.pixmap, &view.mask, &view.attributes);
        ErrorMessage(ErrorStatus, "Read");
    } else {

#ifdef Debug2
        ErrorStatus =
            XpmCreatePixmapFromData(dpy, win, data, &view.pixmap, &view.mask, &view.attributes);
#else
        ErrorStatus =
            XpmCreatePixmapFromData(dpy, win, plaid, &view.pixmap, &view.mask, &view.attributes);
#endif
    ...
    if (output || stdoutf) {
        ErrorStatus = XpmWriteFileFromPixmap(dpy, output, view.pixmap, view.mask, &view.attributes);
    ...
    if (!nod) {
        ...
        if (w_rtn && h_rtn && ((w_rtn < view.attributes.width) || h_rtn < view.attributes.height)) {
            resize = 1;
        }
        if (resize) {
            XtResizeWidget(topw, view.attributes.width, view.attributes.height, 1);
        }
        if (incResize) {
        ...
        XtAddEventHandler(topw, KeyPressMask, False, (XtEventHandler)kinput, NULL);
...
void ErrorMessage(ErrorStatus, tag) int ErrorStatus;
char* tag;
{
    char* error = NULL;
    char* warning = NULL;
...
void Punt(i) int i;
{
    if (icon.pixmap) {
        ...
        XFreeColors(dpy, colormap, icon.attributes.pixels, icon.attributes.npixels, 0);
...
void kinput(widget, tag, xe, b) Widget widget;
char* tag;
XEvent* xe;
Boolean* b;
{

	...
	case "setStyles":
		...
		if (tagPtr) {
			...
			/* Get current font (H2 default is bold_large=5) */
			currentFont = get("_font");
			if (currentFont == 0) currentFont = 5;
			
			/* Extract slant and size from current font */
			if (currentFont == 0 && (fontSlant != 0 || fontSize != 0)) slant = "bold"; /* H2 default */

        ...
        if (slotc > 0) {
            if ((obj = instantiateObj(&slotv, &slotc))) {
                ...
                if (newObjCount > NEWOBJ_SIZE) {
                    fprintf(stderr, "Internal error: newObj[] overflowed.\n");
                }
            } else {
                fprintf(stderr, "failed to build object...\n");
            }
        }
    }
        ...
        if (seekBackPosition) {
            ...
            */
            ...
            if (sloti - dynaBuffi > 0) {
                if (fseek(fp, seekBackPosition - 1, SEEK_SET) == -1)
                    return 0;
                if (fread(&dynaBuff[dynaBuffi], sizeof(char), sloti - dynaBuffi + 2, fp) == 0)
                    return 0;
            }
            dynaBuff[sloti - 1] = '\0';
            /*			printf("dynabuff=\"%s\"\n", dynaBuff);*/
...
int load_objects_slots_fromBuiltInCache(SlotStruct* slotsInfo, int* slotsInfoIdx, long (*slotv)[100][2], int* slotc)
{
    ...
    for (;;) {
        ...
        */
        ...
        /* read the slot content
         */
        (*slotv)[*slotc][1] = (long)saveString(slotsInfo[(*slotsInfoIdx)++].slotValue);
    ...
    while (status) {
        ...
        if (slotc > 0) {
            if ((obj = instantiateObj(&slotv, &slotc))) {
                ...
                objID2Obj->put_replace(objID2Obj, storeIdent(saveString(objname)), (long)obj);
                objObj2ExistP->put_replace(objObj2ExistP, (long)obj, 1);
                newObj[newObjCount++] = obj;
                if (newObjCount > NEWOBJ_SIZE) {
                    fprintf(stderr, "Internal error: newObj[] overflowed.\n");

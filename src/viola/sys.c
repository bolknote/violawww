...
char* _Vmalloc(MemoryGroup* group, int size)
{
    if (!group->heap) {
        ...
        if (size > group->defaultSize)
            group->heap->size = size;
        else
            group->heap->size = group->defaultSize;
    ...
    group->heapUsedOffset += 4 - ((int)(group->heap->mem + group->heapUsedOffset) % 4);
    if (group->heapUsedOffset + size <= group->heap->size) {
        ...
        printf("(0x%x) OLD group=0x%x heap=0x%x mem=0x%x memSZ=%d: iB=%d size=%d iA=%d\n", ptr,
               group, group->heap, group->heap->mem, group->heap->size, group->heapUsedOffset, size,
        ...
        MemoryGroupHeap* newHeap = (MemoryGroupHeap*)malloc(sizeof(struct MemoryGroupHeap));
        if (!newHeap) {
            perror("malloc failed in Vmalloc()");
            return NULL;
        }
        if (size > group->defaultSize)
            ...
            newHeap->size = group->defaultSize;
        ...
        printf("(0x%x) NEW group=0x%x heap=0x%x mem=0x%x memSZ=%d: iB=%d size=%d iA=%d\n",
               newHeap->mem, group, group->heap, newHeap->mem, group->heap->size,
               group->heapUsedOffset, size, size);
#endif
        group->heapUsedOffset = size;
        return newHeap->mem;
    }
...
void _Vfree(group, ptr) MemoryGroup* group;
char* ptr;
{
/*
FOR THE FUTURE:
...
#ifdef MEM_USE
void freeMemoryGroup(group) MemoryGroup* group;
{
    if (group) {
        if (group->heap)
            freeMemoryGroupHeaps(group->heap);
        free(group);
    }
}
#endif
...
#ifdef MEM_USE
void freeMemoryGroupHeaps(group) MemoryGroup* group;
{

...
static Image* bitmapToBitmap(Image* src, Image* dest, unsigned int atx, unsigned int aty, unsigned int clipw, unsigned int cliph, unsigned int verbose)
{
    ...
    flip =
        ((*dest->rgb.red == *(src->rgb.red + 1)) && (*dest->rgb.green == *(src->rgb.green + 1)) &&
...
Image* merge(Image* dest, Image* src, int atx, int aty, unsigned int verbose)
{
    ...
    clipw = src->width;
    cliph = src->height;
    if ((atx + clipw < 0) || (aty + cliph < 0) || (atx >= (int)dest->width) ||
    ...
    if (BITMAPP(dest) && BITMAPP(src))
        newimage =
            bitmapToBitmap(src, dest, (unsigned int)atx, (unsigned int)aty, clipw, cliph, verbose);
    else
        newimage =
            anyToTrue(src, dest, (unsigned int)atx, (unsigned int)aty, clipw, cliph, verbose);
    if (clipped)
        freeImage(src);
    return (newimage);
}

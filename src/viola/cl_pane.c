...
SlotInfo cl_pane_NPSlots[] = {{STR_paneConfig, PTRS | SLOT_RW, (long)"freeForm"},
                              {STR__paneConfig, LONG, PANE_CONFIG_FREE},
                              {0}};
SlotInfo cl_pane_CSlots[] = {{STR_class, PTRS | SLOT_RW, (long)"pane"},
                             {
                                 STR_classScript,
                                 PTRS,
                                 (long)"\n\
...
",
                             ...
                             {0}};
SlotInfo cl_pane_PSlots[] = {{STR__classInfo, CLSI, (long)&class_pane}, {0}};

SlotInfo* slots_pane[] = {(SlotInfo*)cl_pane_NCSlots, (SlotInfo*)cl_pane_NPSlots,
                          (SlotInfo*)cl_pane_CSlots, (SlotInfo*)cl_pane_PSlots};

MethodInfo meths_pane[] = {
    /* local methods */
    {STR_clone, meth_pane_clone},
    {STR_clone2, meth_pane_clone2},
    {STR_config, meth_pane_config},
    {
        STR_expose,
        meth_pane_expose,
    },
    ...
    {STR_initialize, meth_pane_initialize},
    {STR_render, meth_pane_render},
    {STR_seta, meth_pane_set},
    {0}};

ClassInfo class_pane = {
    helper_pane_get, helper_pane_set, slots_pane, /* class slot information	*/
...
helper_pane_config(self) VObj* self;
{
    ...
    if (doit) {
        ...
        for (olist = children; olist; olist = olist->next) {
            if (nchildren < CHILD_MAX) {
                ...
                fprintf(stderr, "Internal error: pane config buffer overflowed.\n");
                break;
            }
            nchildren++;
        }
        ...
        if (positive) {
            for (i = 0; i < nchildren; i++) {
                if (!CHILD_SPAN(i))
                    CHILD_SPAN(i) = span - CHILD_GAP(i) * 2;
                CHILD_SPANO(i) = self[spanOtherIdx] - CHILD_GAPO(i) * 2;
                CHILD_POSN(i) = position - 1 + CHILD_GAP(i);
                position += CHILD_SPAN(i) + 1 + CHILD_GAP(i) * 2;
            }
        } else {
    ...
    if (posIdx == 1) {
        for (i = 0, olist = children; olist; olist = olist->next, i++) {
            ...
            */
            if (verbose)
                fprintf(stderr, "pane y: %s : %ld %ld %ld %ld\n", GET_name(olist->o), GET_x(olist->o),
            ...
            if (GET_window(olist->o))
                if (CHILD_SPANO(i) >= 1 && CHILD_SPAN(i) >= 1) {
                    GLUpdateGeometry(0, GET_window(olist->o), CHILD_GAPO(i), CHILD_POSN(i),
        ...
        for (i = 0, olist = children; olist; olist = olist->next, i++) {
            ...
            */
            if (verbose)
                fprintf(stderr, "pane x: %s : %ld %ld %ld %ld\n", GET_name(olist->o), CHILD_POSN(i),
            ...
            if (GET_window(olist->o))
                if (CHILD_SPANO(i) >= 1 && CHILD_SPAN(i) >= 1) {
                    GLUpdateGeometry(0, GET_window(olist->o), CHILD_POSN(i), CHILD_GAPO(i),
...
long meth_pane_get(VObj* self, Packet* result, int argc, Packet argv[]) {
    return helper_pane_get(self, result, argc, argv, getIdent(PkInfo2Str(argv)));
}
...
long meth_pane_initialize(VObj* self, Packet* result, int argc, Packet argv[]) {
    ...
    if (!meth_field_initialize(self, result, argc, argv))
        return 0;
...
long meth_pane_set(VObj* self, Packet* result, int argc, Packet argv[]) {
    return helper_pane_set(self, result, argc, argv, getIdent(PkInfo2Str(argv)));
}

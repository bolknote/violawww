...
XpmAttributes* attributes;
{
     ...
     */
    sprintf(buf, "%d %d %d %d", attrib->width, attrib->height, attrib->ncolors, attrib->cpp);

    if (attributes && (attributes->valuemask & XpmHotspot))
        sprintf(&buf[strlen(buf)], " %d %d", attributes->x_hotspot, attributes->y_hotspot);
        ...
        if (attributes && (attributes->valuemask & XpmInfos) &&
            ...
            for (key = 1; key < NKEYS + 1; key++) {
                if (attributes->colorTable[attributes->mask_pixel][key])
                    sprintf(&buf[strlen(buf)], "\t%s %s", xpmColorKeys[key - 1],
    ...
    for (a = d; a < attrib->ncolors; a++) { /* other colors */
        ...
        if (c) {
            ...
            if (colorname)
                ...
                sprintf(&buf[strlen(buf)], "\tc #%04X%04X%04X", attrib->xcolors[a].red,
                        attrib->xcolors[a].green, attrib->xcolors[a].blue);
        }
    ...
    *data_return = (char**)malloc(data_size);
     ...
     * print pixels
     */
    (*data_return)[header_nlines] =
        (char*)*data_return + header_size + attrib->height * sizeof(char*);
    ...
    for (y = 0; y < attrib->height; y++) {
        ...
        if (y < attrib->height - 1)
            (*data_return)[header_nlines + y + 1] =
                (*data_return)[header_nlines + y] + attrib->width * attrib->cpp + 1;
    }

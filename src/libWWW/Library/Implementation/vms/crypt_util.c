 ...
 */
static int pc1[56] = {57, 49, 41, 33, 25, 17, 9,  1,  58, 50, 42, 34, 26, 18, 10, 2,  59, 51, 43,
 ...
 * of the i' DES round
 */
static int pc2[48] = {14, 17, 11, 24, 1,  5,  3,  28, 15, 6,  21, 10, 23, 19, 12, 4,
 ...
 */
static int esel[48] = {32, 1,  2,  3,  4,  5,  4,  5,  6,  7,  8,  9,  8,  9,  10, 11,
 ...
 */
static int perm32[32] = {16, 7, 20, 21, 29, 12, 28, 17, 1,  15, 23, 26, 5,  18, 31, 10,
 ...
 * permutation matrix
 */
static int initial_perm[64] = {58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4,
 ...
 * permutation matrix
 */
static int final_perm[64] = {40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31,
...
static unsigned char bytemask[8] = {0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01};
...
static set_bits(v, b) ufc_long v;
ufc_long* b;
{
    ufc_long i;
    *b = 0;
    ...
    for (sg = 0; sg < 4; sg++) {
        ...
        for (j1 = 0; j1 < 64; j1++) {
            ...
            for (j2 = 0; j2 < 64; j2++) {
                ...
                to_permute = (((ufc_long)s1 << 4) | (ufc_long)s2) << (24 - 8 * (ufc_long)sg);
...
#ifdef _UFC_64_
                inx = ((j1 << 6) | j2);
                sb[sg][inx] = ((long64)eperm32tab[0][(to_permute >> 24) & 0xff][0] << 32) |
                              (long64)eperm32tab[0][(to_permute >> 24) & 0xff][1];
                sb[sg][inx] |= ((long64)eperm32tab[1][(to_permute >> 16) & 0xff][0] << 32) |
                               (long64)eperm32tab[1][(to_permute >> 16) & 0xff][1];
                sb[sg][inx] |= ((long64)eperm32tab[2][(to_permute >> 8) & 0xff][0] << 32) |
...
STATIC void setup_salt(s) char* s;
{
...
char* crypt(char * key, char * salt)
{
     ...
     */
    s = _ufc_doit((ufc_long)0, (ufc_long)0, (ufc_long)0, (ufc_long)0, (ufc_long)25);
    /*
...
void encrypt(block, edflag) char* block;
int edflag;
{
...
char* crypt16(char * key, char * salt)
{
     ...
     */
    s = _ufc_doit((ufc_long)0, (ufc_long)0, (ufc_long)0, (ufc_long)0, (ufc_long)20);
     ...
     */
    t = _ufc_doit((ufc_long)0, (ufc_long)0, (ufc_long)0, (ufc_long)0, (ufc_long)5);
    /*
     * And convert back to 6 bit ASCII
     */
...
void ufc_setup_password(cookie, s) long* cookie;
char* s;
{
    char c;
    int i;
...
void ufc_do_pw(cookie, guess) long* cookie;
char* guess;
{
    char ktab[9];
    ufc_long* s;

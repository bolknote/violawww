...
MethodInfo meths_stack[] = {
    /* local methods */
    {STR_anyCard, meth_stack_anyCard},
    {STR_card, meth_stack_card},
    {STR_create, meth_stack_create},
    {STR_config, meth_stack_config},
    {STR_expose, meth_stack_expose},
    {STR_firstCard, meth_stack_firstCard},
    {STR_geta, meth_stack_get},
    {STR_getCurrentCard, meth_stack_getCurrentCard},
    {STR_initialize, meth_stack_initialize},
    {STR_lastCard, meth_stack_lastCard},
    {STR_nextCard, meth_stack_nextCard},
    {STR_countBCards, meth_stack_countBCards},
    {STR_countFCards, meth_stack_countFCards},
    {STR_previousCard, meth_stack_previousCard},
...
Packet argv[];
{
    ...
    if (cp)
        if (*cp)
            SET__BCardList(self, strOListToOList(cp));

    cp = GET_FCardList(self);
    if (cp)
        if (*cp)
            SET__FCardList(self, strOListToOList(cp));
...
Packetp result;
{
    ...
    if (BCList = ObjBCardList(self)) {
        ...
        if (verbose)
            fprintf(stderr, "no BCards in stack.\n");
    }
    clearPacket(result);
...
Packetp result;
{
    ...
    if (stackObj) {
        ...
        if (verbose)
            fprintf(stderr, "method_stack_firstCard(self='%s'): no stack object found.",
                    ObjName(self));
    }
    return PkObject(result);
}
...
Packetp result;
{
    ...
    if (!method_generic_lastObjInList(stackObj, result, "cardList"))
        if (verbose)
            fprintf(stderr, "lastCard: no card in cardList of stack.\n");
    return PkObject(result);
}

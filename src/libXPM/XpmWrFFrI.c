...
int XpmWriteFileFromImage(Display* display, char* filename, XImage* image, XImage* shapeimage, XpmAttributes* attributes)
{
    ...
    if (filename) {
        ...
        if (end_name = strchr(name, '.')) {
            ...
            if (!name) {
                ...
                strncpy(name, begin_name, (unsigned int)(end_name - begin_name));
     ...
     */
    ErrorStatus = xpmScanImage(display, image, shapeimage, attributes, &attrib);
...
static int WriteData(xpmData* mdata, xpmInternAttrib* attrib, XpmAttributes* attributes)
{
     ...
     */
    fprintf(mdata->stream.file, "%s XPM %s\n", xpmDataTypes[n].Bcmt, xpmDataTypes[n].Ecmt);
    if (n != 0) /* print the assignment line */
        fprintf(mdata->stream.file, "%s %s %s", xpmDataTypes[n].Dec, attrib->name,
                xpmDataTypes[n].Boa);

    /*
     * print the hints line
     */
    if (attributes && (attributes->valuemask & XpmInfos) && attributes->hints_cmt)
        /* print hints comment line */
        fprintf(mdata->stream.file, "%s%s%s\n", xpmDataTypes[n].Bcmt, attributes->hints_cmt,
                xpmDataTypes[n].Ecmt);

    if (xpmDataTypes[n].Bos)
        fprintf(mdata->stream.file, "%c", xpmDataTypes[n].Bos);

    fprintf(mdata->stream.file, "%d %d %d %d", attrib->width, attrib->height, attrib->ncolors,
            attrib->cpp);

    if (attributes && (attributes->valuemask & XpmHotspot))
        fprintf(mdata->stream.file, " %d %d", attributes->x_hotspot, attributes->y_hotspot);
     ...
     * print colors
     */
    if (attributes && (attributes->valuemask & XpmInfos) && attributes->colors_cmt)
        /* print colors comment line */
        fprintf(mdata->stream.file, "%s%s%s\n", xpmDataTypes[n].Bcmt, attributes->colors_cmt,
                xpmDataTypes[n].Ecmt);
        ...
        if (attributes && (attributes->valuemask & XpmInfos) &&
            ...
            for (key = 1; key < NKEYS + 1; key++) {
                if (attributes->colorTable[attributes->mask_pixel][key])
                    fprintf(mdata->stream.file, "\t%s %s", xpmColorKeys[key - 1],
    ...
    for (a = d; a < attrib->ncolors; a++) { /* other colors */
        ...
        if (c) {
            colorname = NULL;
            if (rgbn_max)
                colorname = xpmGetRgbName(rgbn, rgbn_max, attrib->xcolors[a].red,
                                          attrib->xcolors[a].green, attrib->xcolors[a].blue);
            if (colorname)
                fprintf(mdata->stream.file, "\tc %s", colorname);
            else
                fprintf(mdata->stream.file, "\tc #%04X%04X%04X", attrib->xcolors[a].red,
                        attrib->xcolors[a].green, attrib->xcolors[a].blue);
        }
        if (xpmDataTypes[n].Eos)
            fprintf(mdata->stream.file, "%c", xpmDataTypes[n].Eos);
     ...
     * print pixels
     */
    if (attributes && (attributes->valuemask & XpmInfos) && attributes->pixels_cmt)
        /* print pixels comment line */
        fprintf(mdata->stream.file, "%s%s%s\n", xpmDataTypes[n].Bcmt, attributes->pixels_cmt,
                xpmDataTypes[n].Ecmt);
    ...
    for (y = 0; y < attrib->height; y++) {
        ...
        for (x = 0; x < attrib->width; x++, iptr++)
            for (b = 0; b < attrib->cpp; b++)
                fprintf(mdata->stream.file, "%c", attrib->colorStrings[*iptr][b]);

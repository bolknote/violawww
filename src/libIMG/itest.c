...
char* filename;
{
    ...
    attrs.event_mask =
        StructureNotifyMask | KeyPressMask | ButtonPressMask | ButtonReleaseMask | ExposureMask;

    winObj[winObjIdx].node = ip;
    winObj[winObjIdx].w =
        XCreateWindow(Disp, RootWindow(Disp, Scrn), (int)10 + (i * 50), (int)10, (int)ip->width,
                      (int)ip->height, (int)1, /* border thickness */
                      CopyFromParent, CopyFromParent, CopyFromParent, CWEventMask, &attrs);
    ...
    case Expose: {
        ...
        for (i = 0; i < winObjIdx; i++) {
            if (winObj[i].w == w) {
                ...
                XPutImage(Disp, w, gc, winObj[i].node->ximageinfo->ximage, 0, 0, 0, 0,
                          winObj[i].node->width, winObj[i].node->height);
                XFlush(Disp);
                break;
            }
        }

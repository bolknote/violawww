...
char* findTagName(int tagID)
{
    ...
    if (tagID > tagDictCount)
        return NULL;
    return tagDict[tagID];
}

build(fp, tagID) FILE* fp;
int tagID;
{
    char line[100000];
    char attrName[64];
    ...
    do {
        ...
        case TOKEN_ATTR: {
            ...
            if (attrTypeID > 0) {
                ...
                fprintf(stdout, "Attr: type=%s name={%s} value={%s}\n",
                        sgmlAttrTypesStr[attrTypeID], attrName, line);
            } else {
                fprintf(stdout, "Attr: type=%s name={%s}\n", sgmlAttrTypesStr[attrTypeID],
                        attrName);
            }
        } break;
        case TOKEN_DATA: {
